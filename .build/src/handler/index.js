...
const convo = new import_telegraf.Scenes.BaseScene("convo");
...
convo.on((0, import_filters.message)("text"), async (ctx) => {
  const text = ctx.message.text;
  ...
  try {
    await ctx.replyWithMarkdown((0, import_domain.constructTextResponse)(response));
  } catch (err) {
    console.error(err);
    await ctx.reply((0, import_domain.constructTextResponse)(response));
  }
